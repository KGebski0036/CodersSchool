...
class AddEven : public ::testing::TestWithParam<std::tuple<std::vector<int>,
	int>>
{};
...
TEST_P(AddEven, ShouldAddEvenNumbers)
{
	...
	expected_result = std::get<1>(GetParam());
	EXPECT_EQ(addEven(std::vector<int>(vec.begin(), vec.end())),
		expected_result);
}

INSTANTIATE_TEST_SUITE_P(AddEvenTest, AddEven,
